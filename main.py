...
class BlogPost(db.Model):
    ...
    post_comments = relationship("Comment", back_populates="posts")
...
def register():
    ...
    if register_form.validate_on_submit():
        duplicate_user = db.session.query(User).filter_by(email=register_form.email.data).first()
        if duplicate_user:
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            flash("No such user, please register for an account!")
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        new_comment = Comment(
            body=comment_form.comment_text.data,
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("post.html", post=requested_post, form=comment_form, logged_in=current_user.is_authenticated, comments=comments)
